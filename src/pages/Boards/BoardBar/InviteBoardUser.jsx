...
function InviteBoardUser({ boardId }) {
  ...
  */
  ...
  const submitInviteUserToBoard = (data) => {
    const { invitedEmail: invitedEmail } = data

    // console.log('invitedEmail:', invitedEmail)
    // Go iAPI moi mot nguoi dung nao do vao lam thanh vien cua Board
    inviteUserToBoardAPI({ invitedEmail, boardId }).then(() => {
      // Clear thẻ input sử dụng react-hook-form bằng setValue, dong thoi dong popover lai
        ...
        <form onSubmit={handleSubmit(submitInviteUserToBoard)} style={{ width: '320px' }}>
          <Box sx={{ p: '15px 20px 20px 20px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box>
              ...
              />
              <FieldErrorAlert errors={errors} fieldName={'invitedEmail'} />
            </Box>
